...
def log_on_slack(s_client,a_client, data):
    ...
    for dat in data:
        ...
        s3_name = f"test/{names}_{now}.jpg"
    ...
    if len(faces)>=1:
        for face in faces:
            ...
            encoding = (DeepFace.represent(img_path=face_array, detector_backend='skip',
                            enforce_detection=False, model_name='Dlib'))
            analysis = DeepFace.analyze(img_path = face_array, actions = ["emotion",],enforce_detection=False,detector_backend='skip')
            print(analysis)
...
def find_closest_face(unknown_face, faces):
    ...
    # if min(distances)<0.51:  
        ...
        if  (distances_sort[1]- distances_sort[0])/ min(distances) >0.21:
          unknown_face['name'] = faces["name"][distances.index(distances_sort[0])] 
          unknown_face['confidence'] = 1 
    elif min(distances)<0.56:
        if  (distances_sort[1]- distances_sort[0])/ min(distances) >0.1:
            unknown_face['name'] = faces["name"][distances.index(distances_sort[0])] 
